...
public class blue_move : Agent
{
    ...
    public override void OnActionReceived(ActionBuffers actions) {
        ...
        if (2<lapsetime && lapsetime<12)
        {
            ...
            rBody.AddForce(dirToGo * 6.0f, ForceMode.VelocityChange);
            //攻撃
            if (action == 6) 
            {
                this.tag = "attack";
    ...
    void Update()
    {
        ...
        if (0<=lapsetime && lapsetime<=12)
            boost = 1.0f-lapsetime/12;
...
//mlagents-learn config/exvs_high-low.yaml --run-id=exvs_high-low --env=apps/exvs_high-low --height=900 --width=1600 --force
//mlagents-learn config/exvs_high-low.yaml --run-id=exvs_high-low --force